...
def upload(client: Client, filepath: str, verbose: bool = False) -> str:
    ...
    partial_file_id = start_partial_res.get_parsed_data()["payload"]["id"]
    ...
    file_id = complete_partial_res.get_parsed_data()["payload"]["id"]
    ...
    sha1_server = digest_res.get_parsed_data()["payload"]["digest"]
...
def __start_partial(client: Client, filename: str, byte_count: int):
    try:
...
def __append_partial(client: Client, partial_file_id: str, content: bytes, start: int, stop: int):
    try:
        return client.send_request(
            "uploaded_files",
            "append_partial",
...
def __complete_partial(client: Client, partial_file_id: str):
    try:
        return client.send_request(
            "uploaded_files",
            "complete_partial",
...
def __digest_server(client: Client, file_id: str):
    try:
        return client.send_request(
            "uploaded_files",
            "digest",
...
def __digest_client(content: bytes):
    try:
        hash = hashlib.sha1()
        hash.update(content)
